...
def get_portfolio_data(val):
    ...
    df_tm = df_tm[['ticker', 'name', 'currency_x', 'Qty', 'quote_x', 'new_gbp', 'delta', 'pct_gain', 'sector', 'industry', 'owner', 'stock_style']]
    df_tm.columns = ['Ticker', 'Company Name', 'CCY', 'Qty', 'Quote', 'GBP Value', 'GBP Gain', '% Gain', 'Sector', 'Industry', 'Owner', 'Style']
...
layout = html.Div([
    html.H1('TBIC Shares by Type', style={'textAlign': 'center'}),
    dbc.Row([
        dbc.Col(
            dcc.Slider(min=1, max=6, marks=slider_dict, id="stype-treemap-slider", value=1),
    ...
    dbc.Spinner(dcc.Graph(id='stype-treemap', figure = {}), color='primary'),
])
@app.callback(Output('stype-treemap', 'figure'),
              [Input('stype-treemap-slider', 'value')])
def currency_treemap(value):
    tbic_port = get_portfolio_data(value)

    tbic_port["portfolio"] = "The TBIC Portfolio - By Type"
    fig = px.treemap(tbic_port, path=['portfolio', 'Style', 'CCY', 'Sector', 'Industry', 'Company Name'],
